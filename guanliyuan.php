...
<body>
<h1 style="text-align: center;color: #000000;">管理员信息更新</h1>
...
$sql = "select mid,username,password from monitor where mid='$number'";
$result = mysqli_query($conn, $sql);
$row = @mysqli_fetch_array($result);
if (empty($row)) {
    $row = array('mid' => '', 'username' => '', 'password' => '','h_mid' => '');
}
...
<form name="frm2" method="post" enctype="multipart/form-data">
    <table border="1" align="center">
        <tr>
            ...
            <td>
                <input name="mid" type="text" value="<?php echo $row['mid']; ?>">
                <input name="h_mid" type="hidden" value="<?php echo $row['mid']; ?>">
            </td>
        </tr>
...
if (@$_POST["b"] == '修改')
{
    if ($num!=$SFZH){
        ...
        $update_sql="update monitor set username='$YHM',password='$MM'  where mid='$SFZH'";
       $update_result=  mysqli_query($conn,$update_sql);
       if (mysqli_affected_rows($conn) != 0){
            echo "<script>alert('修改成功!');location.href='guanliyuan.php'</script>";

        }  else {
            echo "<script>alert('修改失败!');location.href='guanliyuan.php'</script>";
        }
    }
}
...
if (@$_POST["b"] == '添加') {
     ...
     $insert_sql = "insert into monitor(mid,username,password) values('$num','$YHM','$MM') ";

        $insert_result = mysqli_query($conn, $insert_sql);
        if (mysqli_affected_rows($conn) != 0){
            echo "<script>alert('添加成功!');location.href='guanliyuan.php'</script>";

        }  else {
            echo "<script>alert('添加失败!');location.href='guanliyuan.php'</script>";
        }
    }
